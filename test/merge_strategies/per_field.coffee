...
describe 'PerField', ->
  ...
  it 'keeps ours if only modified in ours', ->
    ...
    newData = new @Post(id: '1', title: 'titleA', body: 'bodyA', user: new @User(id: '2', posts: [new @Post(id: '1')]), comments: [new @Comment(id: '3', user: new @User(id: '2'), post: new @Post(id: '1'))])
  ...
  it 'still merges model if removed from hasMany in ours', ->
    post = @session.merge new @Post(id: '1', title: 'herp', comments: [new @Comment(id: '2', body: 'herp', post: new @Post(id: '1'))])
    comment = post.comments[0]
    post.comments.clear()
    expect(post.comments.length).to.eq(0)
    @session.merge new @Post(id: '1', title: 'herp', comments: [new @Comment(id: '2', body: 'derp', post: new @Post(id: '1'))])
    expect(post.comments.length).to.eq(0)
